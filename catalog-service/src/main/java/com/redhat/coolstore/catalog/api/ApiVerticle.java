...
public class ApiVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) throws Exception {
		...
		vertx.createHttpServer().requestHandler(router::accept).listen(this.config().getInteger("catalog.http.port"),
				res -> {
					if (res.succeeded()) {
						startFuture.complete();
					} else {
						startFuture.fail("Server can't start");
					}
				});

		HealthCheckHandler healthCheckHandler = HealthCheckHandler.create(vertx).register("health", f -> health(f));
		
		router.get("/health/readiness").handler(healthCheckHandler);
	...
	private void getProducts(RoutingContext rc) {
		...
		catalogService.getProducts(res -> {
			if (res.failed()) {
				rc.fail(500);
			} else {
				JsonArray _json = new JsonArray(Arrays.asList(res.result().stream().map(p -> p.toJson()).toArray()));
				rc.response().putHeader("content-type", "application/json; charset=utf-8")
						.end(Json.encodePrettily(_json));
			}
		});
	}
	...
	private void getProduct(RoutingContext rc) {
		...
		catalogService.getProduct(rc.request().getParam("itemId"), res -> {
			if (res.failed()) {
				rc.fail(404);
			} else {
				rc.response().putHeader("content-type", "application/json; charset=utf-8")
						.end(Json.encodePrettily(res.result().toJson()));
			}
		});
	}
	...
	private void addProduct(RoutingContext rc) {
		...
		Product p = new Product(rc.getBodyAsJson());

		catalogService.addProduct(p, res -> {
			if (res.failed()) {
				rc.fail(500);
			} else {
	...
	private void health(Future<io.vertx.ext.healthchecks.Status> healthFuture) {
		catalogService.ping(res -> {
			if (res.succeeded()) {
